<template>
  <div v-if="isLoggedIn" class="max-w-screen-lg mx-auto p-6 space-y-8">
    ...
    <div class="card bg-gray-800 text-white rounded-lg p-6 shadow-md">
      ...
      <form @submit.prevent="resetPassword" class="space-y-4">
        <div>
          <label for="current-password" class="block text-sm font-medium">当前密码：</label>
          <input
            type="password"
            id="current-password"
        ...
        <div>
          <label for="new-password" class="block text-sm font-medium">新密码：</label>
          <input
            type="password"
            id="new-password"
        ...
        <div>
          <label for="confirm-password" class="block text-sm font-medium">确认新密码：</label>
          <input
            type="password"
            id="confirm-password"
            ...
            required
          />
          <span class="text-red-500 text-sm mt-1 block" v-if="confirmPasswordError">{{ confirmPasswordError }}</span>
        </div>
  ...
  <div v-else class="max-w-screen-lg mx-auto p-6 text-center">
    ...
    <router-link to="/login" class="mt-4 inline-block bg-blue-500 text-white px-4 py-2 rounded-md hover:bg-blue-600 transition-colors">
      登录
    </router-link>
  </div>
</template>
  ...
  setup() {
    ...
    const resetPassword = async () => {
      ...
      try {
        ...
        console.log(error);
        const errorMessage = error.response?.data?.error || '密码修改失败，请稍后重试';
        showNotification({
      ...
      if (accessToken) {
        try {
          ...
          if (response.data.isAuthenticated) {
            ...
            email.value = response.data.email || 'user@example.com';
